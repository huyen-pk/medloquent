...
- Runs the Python synthetic pipeline (`synthetic/cli.py`) to extract text, synthesize audio using Coqui TTS, run ASR (Vosk/Whisper), and evaluate.
...
Notes:
- The pipeline image installs the full `requirements-full.txt` (Coqui TTS, Vosk and related libs). There is no deterministic TTS/ASR fallback anymore — the build will be significantly larger and may take a long time to complete on first build.
...
- Outputs are persisted in Docker named volumes `synthea_output` and `synthetic_output`.
